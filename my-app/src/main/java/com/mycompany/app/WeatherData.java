...
public class WeatherData extends AbstractWeatherData {
	...
	/* */
	//cuz this ish in kelvin 
	public float getTemp () {
		if (hasMain () && this.main.hasTemp ())
			return Math.round((this.main.getTemp () * 9f) / 5f - 459.67f);
		return Float.NaN;
	}
	...
	public float getTempMax () {
		if (hasMain () && this.main.hasTempMax ())
			return Math.round((this.main.getTempMax () * 9f) / 5f - 459.67f);
		return Float.NaN;
	}
	
	public float getTempMin () {
		if (hasMain () && this.main.hasTempMin ())
			return Math.round((this.main.getTempMin () * 9f) / 5f - 459.67f);
		return Float.NaN;
	}
	...
	//originally in meters per sec
	//changed to miles per hour
	public float getWindSpeed () {
		if (hasWind () && this.wind.hasSpeed ())
			return Math.round(this.wind.getSpeed () * 2.23694f * 100) / 100f;
		return Float.NaN;
	}
